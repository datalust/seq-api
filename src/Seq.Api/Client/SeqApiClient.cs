...
public sealed class SeqApiClient : IDisposable
{
    ...
    public SeqApiClient(string serverUrl, string apiKey = null, Func<CookieContainer, HttpMessageHandler> createHttpMessageHandler = null)
    {
        ...
        HttpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue(SeqApiVersionedMediaType));
    ...
    {
        var linkUri = EnsureAbsoluteWebSocketUri(ResolveLink(entity, link, parameters), HttpClient.BaseAddress);
        ...
        foreach (var header in HttpClient.DefaultRequestHeaders)
        {
            ...
            socket.Options.SetRequestHeader(header.Key, System.Linq.Enumerable.FirstOrDefault(header.Value));
        }
        ...
        using var reader = new StreamReader(current, encoding);

        if (message is not NoMessage)
        {
            var w = new StreamWriter(current, encoding);
            serialize(w, message);
            // ReSharper disable once MethodHasAsyncOverload MethodHasAsyncOverloadWithCancellation
            w.Flush();
    ...
    StringContent MakeJsonContent(object content)
    {
    ...
    static string EnsureAbsoluteWebSocketUri(string target, Uri apiBaseAddress)
    {
        UriBuilder absoluteUnknownScheme;
        
        if (new Uri(target, UriKind.RelativeOrAbsolute).IsAbsoluteUri)
        {
            absoluteUnknownScheme = new UriBuilder(target);
        }
        else
        {
            var combined = apiBaseAddress.ToString();
            combined = combined.TrimEnd('/');
            if (!target.StartsWith(
#if STRING_STARTSWITH_CHAR
                    '/'
#else
                    "/"
#endif
    ...
    internal void UpdateCsrfToken(string csrfToken)
    {
        HttpClient.DefaultRequestHeaders.Remove(CsrfTokenHeaderName);
            
        if (!string.IsNullOrEmpty(csrfToken))
        {
            HttpClient.DefaultRequestHeaders.Add(CsrfTokenHeaderName, csrfToken);
        }
    }
}
